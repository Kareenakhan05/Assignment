...
document.addEventListener("DOMContentLoaded", function() {
  var projectItems = document.querySelectorAll(".project1-item");

  projectItems.forEach(function(item) {
      item.addEventListener("click", function() {
          var imageSource = this.getAttribute("data-image");
          document.getElementById("projectImage").src = imageSource;
      });
  });
});
// script.js
const slides = document.querySelector(".slider-images");
  ...
  const showSlide = () => {
      slides.style.transform = `translateX(-${currentSlide * 33.33}%)`; // Adjust according to the number of slides

      dots.forEach((dot, index) => {
          if (index === currentSlide % dots.length) {
